...
class DNSCapture:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self.etw_provider = create_etw_provider(
            event_callback=self._on_event, use_fake=args.fake_provider
        )
    ...
    def start(self) -> None:
        ...
        if self.event_filter.has_filters():
            ...
            if self.args.filter_domain:
                print(
                    f"  Domain: {', '.join(self.args.filter_domain)}", file=sys.stderr
                )
            if self.args.filter_type:
            ...
            if self.args.include_process:
                print(
                    f"  Include process: {self.args.include_process}", file=sys.stderr
                )
            if self.args.exclude_process:
                print(
                    f"  Exclude process: {self.args.exclude_process}", file=sys.stderr
                )
...
Examples:
  ...
  python -m etw_dns --debug-etw --log-level DEBUG
    ...
    parser.add_argument("--version", action="version", version=f"etw_dns {VERSION}")

    output_group = parser.add_argument_group("Output Options")
    output_group.add_argument(
        "--out", metavar="PATH", help="Output file path (default: stdout)"
    )
    output_group.add_argument(
        "--format",
        choices=["jsonl"],
