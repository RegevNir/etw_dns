...
def check_privileges() -> None:
    ...
    if not is_windows():
        ...
        print("ETW (Event Tracing for Windows) is only available on Windows.", file=sys.stderr)
        sys.exit(5)
    
    if not is_admin():
        print("Error: Administrative privileges required.", file=sys.stderr)
        print("", file=sys.stderr)
        print("This tool requires administrator rights to subscribe to ETW events.", file=sys.stderr)
        print("Please run this tool from an elevated command prompt or PowerShell.", file=sys.stderr)
        print("", file=sys.stderr)
