...
class EventFilter:
    ...
    def should_include(self, event: DNSEvent) -> bool:
        ...
        if self.domain_patterns and event.query_name:
            if not any(fnmatch.fnmatch(event.query_name.lower(), pattern.lower()) 
    ...
    def has_filters(self) -> bool:
        ...
        return bool(
            self.domain_patterns or
            self.query_types or
            self.statuses or
            self.include_processes or
            self.exclude_processes or
            self.include_pids or
            self.exclude_pids
        )
